...
@Component({
  selector: 'app-hospital-percentage',
  templateUrl: './hospital-percentage.component.html',
  styleUrls: ['./hospital-percentage.component.css']
})
export class HospitalPercentageComponent {
  ...
  changeVendor(event:any){
    ...
    this.VenderId = event;
    this.hospitalService.getVendorPercentage(this.VenderId).subscribe((response)=>{
  ...
  deleteHospitalRow(id:number){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.hospitalService.DeleteVendorPercentage(id).subscribe(response =>{
          if(response.Code == 1){
...
setTitle!:string
  AddNew(val:number){
    //0 for add
    this.setTitle = 'Add New Hospital Percentage';
    this.Openpopup(val)
  }
  ...
  Openpopup(val:any){
    var _popup =  this.dialog.open(HospitalPercentModaalComponent,{
 width:'40%',
 disableClose: true,
//  height:'400px',
